  ...
  "../../node_modules/.pnpm/bottleneck@2.19.5/node_modules/bottleneck/light.js"(exports, module) {
    (function(global2, factory) {
      ...
      Job = class Job {
        ...
        doDrop({ error, message: message2 = "This job has been dropped by Bottleneck" } = {}) {
          if (this._states.remove(this.options.id)) {
            if (this.rejectOnDrop) {
              this._reject(error != null ? error : new BottleneckError$1(message2));
            }
      ...
      LocalDatastore = class LocalDatastore {
        ...
        async __publish__(message2) {
          await this.yieldLoop();
          return this.instance.Events.trigger("message", message2.toString());
        }
      ...
      Bottleneck2 = function() {
        class Bottleneck3 {
          ...
          publish(message2) {
            return this._store.__publish__(message2);
          }
          disconnect(flush = true) {
          ...
          _dropAllQueued(message2) {
            return this._queues.shiftAll(function(job) {
              return job.doDrop({ message: message2 });
            });
          }
          stop(options = {}) {
...
function requireException() {
  ...
  function formatError(exception2, compact) {
    let where = "";
    const message2 = exception2.reason || "(unknown reason)";
...
function requireLoader() {
  ...
  function generateError(state, message2) {
    ...
    return new YAMLException2(message2, mark);
  }
...
var groups = {};
var createGroups = function(Bottleneck2, common2) {
...
function throttling(octokit, octokitOptions) {
  ...
  if (groups.global == null) {
    createGroups(Bottleneck2, common2);
  }
  const state = Object.assign(
    {
...
var MAX_RATE_LIMIT_WAIT_SECONDS = 60;
function shouldRetryRateLimit(retryAfterSeconds, retryCount) {
  return retryAfterSeconds <= MAX_RATE_LIMIT_WAIT_SECONDS && retryCount < MAX_RATE_LIMIT_RETRIES;
}
var FettleOctokit = Octokit.plugin(retry, throttling);
...
function createGitHubClient(options = {}) {
  return new FettleOctokit({
    ...
    throttle: {
      ...
      onRateLimit: (retryAfter, requestOptions) => shouldRetryRateLimit(retryAfter, requestOptions.request?.retryCount ?? 0),
      onSecondaryRateLimit: (retryAfter, requestOptions) => shouldRetryRateLimit(retryAfter, requestOptions.request?.retryCount ?? 0)
    }
  });
}
...
async function describeRulesets(client, repo, rules) {
  const types2 = [...new Set(rules.map((rule) => rule.type).filter(Boolean))].sort();
  ...
  return types2.length > 0 ? `${subject} (${types2.join(", ")})` : subject;
}
